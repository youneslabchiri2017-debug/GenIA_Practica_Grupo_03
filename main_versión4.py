...
def load_game_assets():
    ...
    # --- COMBOS (TAMAÑOS REDUCIDOS AQUI) ---
    combo_path = "ImageCombo" 
    # He reducido los tamaños aprox a la mitad o un poco más pequeños
    ASSETS["images"]["combos"][2] = load_image(f"ImageCombo/ComboX2.png", size=80)  # Antes 150
    ASSETS["images"]["combos"][3] = load_image(f"ImageCombo/ComboX3.png", size=100) # Antes 180
    ASSETS["images"]["combos"][5] = load_image(f"ImageCombo/ComboX5.png", size=120) # Antes 200
    ASSETS["images"]["combos"]["super"] = load_image(f"ImageCombo/ComboSuper.png", size=140) # Antes 250
...
class Crosshair(pygame.sprite.Sprite):
    """Gestiona la mira, animaciones y cooldown de disparo"""
    def __init__(self):
        super().__init__()
        ...
        self.COOLDOWN_TIME = 30 # 0.5 segundos
...
def run_level(level):
    ...
    while running:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1: 
                    ...
                    if shots_remaining > 0:
                        ...
                        # --- LÓGICA DE ACTIVACIÓN DE IMÁGENES DE COMBO ---
                        if hit_something:
                            combo_count += 1
                            if combo_count > max_consecutive_combo: max_consecutive_combo = combo_count
                            
                            c_img = None
                            # CAMBIO: Usamos mouse_pos en lugar de coordenadas fijas
                            spawn_x, spawn_y = mouse_pos
                            
                            # Mostrar imagen específica según el combo
                            if combo_count == 2:
                                c_img = ComboImage(2, spawn_x, spawn_y)
                            elif combo_count == 3:
                                c_img = ComboImage(3, spawn_x, spawn_y)
                            elif combo_count == 5:
                                c_img = ComboImage(5, spawn_x, spawn_y)
                            elif combo_count > 5:
                                # A partir de 6 en adelante, sale SUPER
                                c_img = ComboImage("super", spawn_x, spawn_y)
        ...
        draw_hud() # Texto del combo arriba
        draw_exit_button()
    ...
    while running:
        ...
        if current_state == STATE_START:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    for i in range(1, 4):
                        if pygame.Rect(WIDTH//2 - 100, 150 + (i * 70), 200, 50).collidepoint((mx, my)):
